...
function initCosmos() {
  ...
  var shootingStars = [];
  var shootingTimer = 0;
  var shootingInterval = isMobile ? 350 : 200;
  ...
  function animate() {
    ...
    nebulae.forEach(function (neb) {
      ...
      grad.addColorStop(0, 'hsla(' + neb.hue + ', 60%, 45%, ' + curOp + ')');
      grad.addColorStop(0.5, 'hsla(' + neb.hue + ', 40%, 30%, ' + (curOp * 0.4) + ')');
    ...
    stars.forEach(function (s) {
      ...
      ctx.fillStyle = 'rgba(200, 230, 225, ' + curOp + ')';
      ctx.fill();
      if (s.size > 1) {
        ctx.beginPath();
        ctx.arc(s.x, s.y, s.size * 3, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(42, 157, 143, ' + (curOp * 0.08) + ')';
        ctx.fill();
      }
    });
    ...
    for (var si = shootingStars.length - 1; si >= 0; si--) {
      ...
      grad.addColorStop(0.6, 'rgba(200, 230, 225, ' + (ss.life * 0.4) + ')');
...
function initForm() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    var formData = {
      ...
      phone: phoneVal,
      company: form.querySelector('#form-company') ? form.querySelector('#form-company').value.trim() : '',
      ...
      source: window.__esePage || 'protocol_diplomacy',
...
function trackEvent(eventName, data) {
  ...
  var payload = {
    sessionId: sessionStorage.getItem('ese_session_id') || '',
    page: window.__esePage || 'protocol_diplomacy',
