...
function initCosmos() {
  ...
  for (var n = 0; n < nebulaCount; n++) {
    ...
    nebulae.push({
      ...
      radius: 100 + Math.random() * 200, hue: hue,
      opacity: 0.015 + Math.random() * 0.015,
      driftX: (Math.random() - 0.5) * 0.15, driftY: (Math.random() - 0.5) * 0.1,
      pulse: Math.random() * Math.PI * 2, pulseSpeed: 0.003 + Math.random() * 0.003
    });
  }
  ...
  for (var c = 0; c < constellationCount; c++) {
    constellationStars.push({
      ...
      size: 1.5 + Math.random() * 1, opacity: 0.3 + Math.random() * 0.3
    });
  }
  ...
  function animate() {
    ...
    nebulae.forEach(function(neb) {
      neb.x += neb.driftX; neb.y += neb.driftY;
