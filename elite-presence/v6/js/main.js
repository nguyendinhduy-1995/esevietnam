/**
 * ÉLITE PRESENCE V6 — Main JS
 * Handles: navbar, reveal animations, lightbox, form, FAQ accordion,
 *          smooth scroll, tracking, parallax, count-up
 */
    ...
    if (!ticking) {
      window.requestAnimationFrame(function () {
        ...
        if (scroll < window.innerHeight * 1.5) {
          hero.style.transform = 'scale(' + (1.02 + scroll * 0.00005) + ') translateY(' + (scroll * 0.15) + 'px)';
        }
        ticking = false;
      });
      ticking = true;
...
/* ====== CURSOR GLOW EFFECT ON PRICING CARDS ====== */
  ...
  cards.forEach(function (card) {
    ...
    card.addEventListener('mouseleave', function () {
      if (card.classList.contains('featured')) {
        card.style.background = 'rgba(196,162,101,0.06)';
      } else {
...
function initForm() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    var data = {
      ...
      source: window.__esePage || 'elite_presence_v6',
...
function trackEvent(name, data) {
  ...
  var p = {
    sessionId: sessionStorage.getItem('ese_session_id') || '',
    page: window.__esePage || 'elite_presence_v6',
    event: name,
    data: data || null,
