...
function initForm() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    var formData = {
      ...
      source: window.__esePage || 'elite_presence_v2',
...
function initTracking() {
  if (!sessionStorage.getItem('ese_session_id')) {
    sessionStorage.setItem('ese_session_id', 'ese_v2_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8));
  }
...
function trackEvent(eventName, data) {
  ...
  var payload = {
    sessionId: sessionStorage.getItem('ese_session_id') || '',
    page: window.__esePage || 'elite_presence_v2',
    event: eventName,
    data: data || null,
